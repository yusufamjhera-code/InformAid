  ...
  try {
    ...
    switch (method) {
      case 'GET':
        ...
        res.status(200).json(disabilityTypes);
        break;

      case 'POST':
        // Create a new disability type
        const newDisabilityType = req.body;
        
        // Validate required fields
        if (!newDisabilityType.id || !newDisabilityType.name) {
          return res.status(400).json({ error: 'Missing required fields' });
        }

        // Insert the new disability type
        const result = await db.collection('disability_types').insertOne(newDisabilityType);
        res.status(201).json({ 
